...
pub fn encaps(pk: &[u8]) -> Result<(Vec<u8>, Zeroizing<Vec<u8>>)> {
    let kem = Kem::new(Algorithm::Kyber1024).map_err(|e| anyhow!("{:?}", e))?;
    let pk_obj = kem.public_key_from_bytes(pk).map_err(|e| anyhow!("{:?}", e))?;
...
pub fn decaps(sk: &SecretKey, ct: &[u8]) -> Result<Zeroizing<Vec<u8>>> {
    let kem = Kem::new(Algorithm::Kyber1024).map_err(|e| anyhow!("{:?}", e))?;
    let ct_obj = kem.ciphertext_from_bytes(ct).map_err(|e| anyhow!("{:?}", e))?;
    let sk_obj = kem.secret_key_from_bytes(&sk.0).map_err(|e| anyhow!("{:?}", e))?;
